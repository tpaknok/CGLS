sim_CGLS <- function(comm,V,ef_mean,sd,
  ...
  if (signals_intercept == T & signals_slope==T) {
    comm_beta <- c(t(chol(C_true)) %*% rnorm(n=nrow(comm),mean=ef_mean,sd=sd))
  ...
  ###GLS-optim
  start_GLS <- Sys.time()
  ML.opt<-optim(init_lambda,likelihood.lambda,y=y,X=x,V=V_sp,comm=comm,method="L-BFGS-B",
 ...
 sim_CGLS <- function(comm,V,ef_mean,sd,
  ...
  if (signals_intercept == T & signals_slope==T) {
    ...
    comm_int <- c(t(chol(C_true)) %*% rnorm(n=nrow(comm),mean=ef_mean,sd=sd))
    sr_E_cor <- cor(x,comm_beta*x+comm_int)
    slope_int_cor <- cor(comm_beta*x,comm_int)
    y <- b1*x+comm_beta*x+comm_int #signals in intercept and slope
  } 
  ...
  start_INLA <- Sys.time()
  ML.opt2<-optim(init_lambda,likelihood.lambda.INLA,data=sim_data,V=V_sp,comm=comm,method="L-BFGS-B",prior="pc.prior.auto",
  ...
  INLA_C_lambda_sig <- sign(INLA_C_lambda[,3]) * sign(INLA_C_lambda[,5])*-1
  } else {
  ...
  return(data.frame(p[[2]],p_gls[[2]],p_gls_true[[2]],p_gls_optim[[2]],INLA_C_sig[[2]],INLA_C_true_sig[[2]],INLA_C_lambda_sig[[2]],
                    ...
                    lambda_GLS=lambda_GLS,lambda_INLA = lambda_INLA,lambda_true=lambda_true,
